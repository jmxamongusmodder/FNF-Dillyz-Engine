...
// based off of https://github.com/ShadowMario/FNF-PsychEngine/blob/main/source/FlxUIDropDownMenuCustom.hx & edit of FlxUIDropDownMenu
...
class FlxUIDropDownMenuScrollable extends FlxUIGroup implements IFlxUIWidget implements IFlxUIClickable implements IHasParams
{
	...
	public function new(X:Float = 0, Y:Float = 0, DataList:Array<StrNameLabel>, ?Callback:String->Void, ?Header:FlxUIDropDownHeader,
			?DropPanel:FlxUI9SliceSprite, ?ButtonList:Array<FlxUIButton>, ?UIControlCallback:Bool->FlxUIDropDownMenuScrollable->Void)
	{
		super(X, Y);
		callback = Callback;
		header = Header;
	...
	public override function update(elapsed:Float):Void
	{
		...
		{
			if (list.length > 1 && FlxG.mouse.wheel != 0)
			{
				curScrollPos -= FlxG.mouse.wheel.snapInt(-1, 1);
				curScrollPos = curScrollPos.snapInt(0, list.length - 1);
				updateButtonPositions();
			}
